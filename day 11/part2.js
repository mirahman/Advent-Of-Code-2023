...
try {
    ...
    const getDistance = (one, two) => {
        ...
        for (let r = r1; r < r2; r++) if (emptyRows[r]) result+=999999;
        for (let c = c1; c < c2; c++) if (emptyCols[c]) result+=999999;
    ...
    for(let i = 0;i<coordinates.length;i++) {
        for(let j = i+1;j<coordinates.length;j++) {
            ...
            //console.log(count++, ": ", i+1, j+1, "[", coordinates[i], ", ", coordinates[j], "] => ", tmp);
            totalDistance += tmp;
        }
    }
